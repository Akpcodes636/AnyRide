...
export default function Availability() {
  ...
  const [pickupCoords, setPickupCoords] = useState<Coords | null>(null);
  const { setStep, setActiveRequest } = useDriverStore();
    ...
    <div>
      {isOnline ? (
        <div>
          ...
          <button
            onClick={() => setIsOnline(false)}
            className="cursor-pointer rounded-full w-[129px] h-[48px] bg-[#EF4444] text-white text-[16px] mb-[24px]"
          >
            Go offline
          </button>
          ...
          {requests.map((req) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <div className="w-[6px] h-[6px] bg-[#22C553] rounded-full" />
                      <p className="text-[10px] font-light text-[#02093A]">New Request</p>
                    </div>
                    <h3 className="text-[#02093A] font-bold text-[14px] leading-[120%]">{req.name}</h3>
                  </div>
                </div>
                <p className="text-[#010C4A] font-bold text-[20px]">{req.code}</p>
              </div>
              ...
              <div className="bg-[#F5F5F7] p-3 rounded-lg space-y-2">
                <div className="relative w-full">
                  <LocationSearchInput
                    ...
                    placeholder={req.pickup}
                    className="w-full h-12 text-[12px] pl-9 border-b border-[#E6E6E6] focus:border-b-[#A20602] focus:outline-none"
                    label="Pickup"
                  />
                  <Image src="/images/Map.png" alt="Map Icon" width={14} height={14}
                    className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                </div>
                <div className="relative w-full">
                  <LocationSearchInput
                    ...
                    placeholder={req.dropoff}
                    className="w-full h-12 text-[12px] pl-9 border-[#E6E6E6] focus:border-b-[#A20602] focus:outline-none"
                    label="Destination"
                  />
                  <Image src="/images/Maps.png" alt="Map Icon" width={14} height={14}
                    className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-[#02093A] text-white rounded-lg py-2 mt-3 cursor-pointer"
              >
                View request
              </button>
            </div>
